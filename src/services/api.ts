...
// // New function to download notes as PDF
// export const downloadNotes = async (userId: string) => {
//   try {
...
const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || 'https://vdgargIIITA-notes-mgmt.hf.space' || 'http://localhost:8000';
...
// New function to edit a note
export const editNote = async (noteId: string, newText: string) => {
  try {
    const response = await api.put('/notes/edit', { 
      note_id: noteId,
      new_text: newText
    });
    return response.data;
